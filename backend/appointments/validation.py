...
def validate_appointment_status(
    ...
    current_user: User,
    student_id: Optional[Union[UUID, str]],
    student_satisfaction: Optional[int],
    notes: Optional[str],
) -> None:
    """
    Validate the following:
    - If the appointment is being completed or scheduled, it must have a student
    - If the appointment is being completed, the coach must be the current user
    - If the appointment is being completed, the student satisfaction and notes must be provided
    - If the appointment is open, it cannot have a student, student satisfaction, or notes
    - If the appointment is open or is being scheduled, a student satisfaction must not be present
    """
    if status in [AppointmentStatus.COMPLETED, AppointmentStatus.SCHEDULED]:
        if student_id is None:
            raise ValueError(
                "Appointment must be completed or scheduled with a student"
            )
        else:
        ...
        if status == AppointmentStatus.COMPLETED:
            ...
            if student_satisfaction is None:
                raise ValueError(
                    "Appointment must be completed with student satisfaction"
                )
            ...
            if student_satisfaction is not None:
                raise ValueError(
                    "Appointment cannot be scheduled with student satisfaction"
                )
