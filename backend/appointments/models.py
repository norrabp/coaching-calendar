...
class Appointment(Model):
    ...
    # Required fields
    coach_id: Mapped[UUID] = db.Column(
        UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=False
    )
    appointment_time: Mapped[datetime] = db.Column(db.DateTime)
    status: Mapped[AppointmentStatus] = db.Column(
        db.Enum(AppointmentStatus), default=AppointmentStatus.OPEN, nullable=False
    )

    # Optional fields
    student_id: Mapped[Optional[UUID]] = db.Column(
        UUID(as_uuid=True), db.ForeignKey("users.id"), nullable=True
    )
    notes: Mapped[Optional[str]] = db.Column(db.Text, nullable=True)
    student_satisfaction: Mapped[Optional[int]] = db.Column(
        db.Integer,
        db.CheckConstraint("student_satisfaction >= 1 AND student_satisfaction <= 5"),
        nullable=True,
    )
    ...
    def to_dict(self) -> dict:
        ...
        if self.student:
            # Only after scheduling should we show phone numbers
            base_dict.update(
                {
