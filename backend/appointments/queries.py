...
def get_appointments_for_coach_in_range_query(
    coach_id: str, start_date_exclusive: datetime, end_date_exclusive: datetime
) -> List[Appointment]:
    return Appointment.get_list(
        {
            "coach_id": coach_id,
            "appointment_time": {
                "$gt": start_date_exclusive,
...
def get_open_appointments_query(
    ...
    filter = {
        ...
        "appointment_time": {
            "$gte": start_time_inclusive or datetime.now(timezone.utc)
        },
    }
    if coach_id:
        filter["coach_id"] = coach_id
    return Appointment.get_list_and_paginate(
        filter, sort={"appointment_time": "asc"}, pagination=pagination_info
    )


def get_appointments_for_user_query(
    user: User, query_opts: QueryOpts, as_student: Optional[bool] = None
) -> tuple[List[Appointment], bool]:
    filter = query_opts.filter or {}
    if user.role == UserRole.STUDENT or (
        user.role == UserRole.ROOT and as_student is True
    ):
        filter["student_id"] = user.id
    elif user.role == UserRole.COACH or (
        user.role == UserRole.ROOT and as_student is False
    ):
        filter["coach_id"] = user.id
    else:
        raise ValueError("Must view appointments as a student or coach")
    return Appointment.get_list_and_paginate(
        filter=filter, sort=query_opts.sort, pagination=query_opts.pagination
    )
...
def get_existing_timeslots_by_datetime_query(
    coach: User, start_time: datetime
) -> List[Appointment]:
    end_time = datetime(start_time.year, start_time.month, start_time.day, 17, 0, 0, 0)
    return Appointment.get_list(
        {
            "coach_id": coach.id,
            "appointment_time": {"$gte": start_time, "$lt": end_time},
        }
    )
