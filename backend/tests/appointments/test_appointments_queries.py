...
from backend.appointments.queries import (
    get_appointments_for_coach_in_range_query,
...
def test_get_appointments_for_coach_in_range_query(
    client: FlaskClient, test_coach: User
):
    ...
    appointments = get_appointments_for_coach_in_range_query(
        test_coach.id, datetime(2024, 1, 1, 9, 0, 0), datetime(2024, 1, 1, 13, 0, 0)
    )
    ...
    appointments = get_appointments_for_coach_in_range_query(
        test_coach.id, datetime(2024, 1, 1, 8, 59, 59), datetime(2024, 1, 1, 13, 0, 1)
    )
...
def test_get_open_appointments_query(test_coach: User, test_student: User):
    ...
    appointments, has_more = get_open_appointments_query(
        PaginationInfo(), coach_id=test_coach.id
    )
    appointment_ids = [appt.id for appt in appointments]
    assert len(appointments) == 3
    assert appointment_1.id not in appointment_ids
    ...
    appointments, has_more = get_open_appointments_query(
        PaginationInfo(),
        start_time_inclusive=datetime(2024, 1, 1, 13, 0, 0),
        coach_id=test_coach.id,
    )
    appointment_ids = [appt.id for appt in appointments]
    ...
    appointments, has_more = get_open_appointments_query(
        PaginationInfo(),
        start_time_inclusive=datetime(2024, 1, 1, 9, 0, 0),
        coach_id=test_coach.id,
    )
    appointment_ids = [appt.id for appt in appointments]
    assert len(appointments) == 4
...
def test_get_appointments_for_user_query(
    client: FlaskClient, test_coach: User, test_student: User, test_root: User
):
    test_coach_2 = UserFactory(role=UserRole.COACH).create()
    ...
    appointments, has_more = get_appointments_for_user_query(
        test_root, QueryOpts(), as_student=True
    )
    appointment_ids = [appt.id for appt in appointments]
    assert len(appointments) == 1
    assert appointment_1.id not in appointment_ids
    ...
    appointments, has_more = get_appointments_for_user_query(
        test_root, QueryOpts(), as_student=False
    )
    appointment_ids = [appt.id for appt in appointments]
    assert len(appointments) == 1
    assert appointment_1.id not in appointment_ids
...
def test_get_existing_timeslots_by_datetime_query(test_coach: User, test_student: User):
    ...
    appointment_4 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 2, 13, 0, 0)
    ).create()
    appointment_5 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 1, 10, 0, 0)
    ).create()
    appointment_6 = AppointmentFactory(
        coach_id=test_coach.id,
        appointment_time=datetime(2024, 1, 1, 10, 0, 0),
    ...
    appointment_7 = AppointmentFactory(
        coach_id=test_coach_2.id, appointment_time=datetime(2024, 1, 1, 15, 0, 0)
    ).create()

    appointments = get_existing_timeslots_by_datetime_query(
        test_coach, datetime(2024, 1, 1, 10, 0, 0)
    )
    assert len(appointments) == 4
    assert appointment_1 not in appointments
    assert appointment_2 in appointments
    ...
    appointments = get_existing_timeslots_by_datetime_query(
        test_coach_2, datetime(2024, 1, 1, 10, 0, 0)
    )
    assert len(appointments) == 1
