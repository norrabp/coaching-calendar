...
def test_create_appointment(
    client: FlaskClient, test_coach: User, coach_auth_headers: dict
):
...
def test_create_appointment_invalid_completion(
    client: FlaskClient, test_coach: User, coach_auth_headers: dict
):
    assert Appointment.query.count() == 0
    response = client.post(
...
@time_machine.travel(
    datetime(2024, 1, 1, 10, 0, 0).replace(tzinfo=timezone.utc), tick=False
)
def test_get_open_appointments(
    client: FlaskClient, test_student: User, test_coach: User
):
    ...
    appointment_2 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 1, 11, 0, 0)
    ).create()
    appointment_3 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 1, 13, 0, 0)
    ).create()
    appointment_4 = AppointmentFactory(
        coach_id=test_coach.id,
        appointment_time=datetime(2024, 1, 1, 15, 0, 0),
        status=AppointmentStatus.SCHEDULED,
        student_id=test_student.id,
    ).create()
    appointment_5 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 2, 17, 0, 0)
    ).create()
    appointment_6 = AppointmentFactory(
        coach_id=test_coach_2.id, appointment_time=datetime(2024, 1, 2, 10, 0, 0)
    ).create()
    ...
    assert response.json == {
        "error": "You are not authorized to view open appointments"
    }
    ...
    response = client.post(
        "/appointments/open",
        headers=student_auth_headers,
        json={"pagination_info": {"page": 1, "page_size": 2}},
    )
    assert response.status_code == 200
    ...
    response = client.post(
        "/appointments/open",
        headers=student_auth_headers,
        json={"pagination_info": {"page": 2, "page_size": 2}},
    )
    assert response.status_code == 200
    assert len(response.json["appointments"]) == 2
    appointment_ids = [appt["id"] for appt in response.json["appointments"]]
...
def test_get_my_appointments(
    ...
    coach_auth_headers: dict,
    root_auth_headers: dict,
):
    test_coach_2 = UserFactory(role=UserRole.COACH).create()
    ...
    appointment_6 = AppointmentFactory(
        coach_id=test_coach_2.id, appointment_time=datetime(2024, 1, 1, 10, 0, 0)
    ).create()
    appointment_7 = AppointmentFactory(
        coach_id=test_root.id,
        appointment_time=datetime(2024, 1, 1, 10, 0, 0),
        status=AppointmentStatus.SCHEDULED,
        student_id=test_student.id,
    ).create()
    appointment_8 = AppointmentFactory(
        coach_id=test_coach.id,
        appointment_time=datetime(2024, 1, 1, 10, 0, 0),
        status=AppointmentStatus.SCHEDULED,
        student_id=test_root.id,
    ).create()
    appointment_9 = AppointmentFactory(
        coach_id=test_coach.id,
        appointment_time=datetime(2024, 1, 1, 15, 0, 0),
        status=AppointmentStatus.SCHEDULED,
        student_id=test_student_2.id,
    ).create()
    ...
    response = client.post(
        "/appointments/my", headers=root_auth_headers, json={"as_student": True}
    )
    assert response.status_code == 200
    appointment_ids = [appt["id"] for appt in response.json["appointments"]]
    ...
    response = client.post(
        "/appointments/my", headers=root_auth_headers, json={"as_student": False}
    )
    assert response.status_code == 200
    appointment_ids = [appt["id"] for appt in response.json["appointments"]]
    assert len(response.json["appointments"]) == 1
...
def test_get_appointment_by_id(
    ...
    open_appointment = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 1, 9, 0, 0)
    ).create()
    scheduled_appointment = AppointmentFactory(
        coach_id=test_coach.id,
        appointment_time=datetime(2024, 1, 1, 11, 0, 0),
        status=AppointmentStatus.SCHEDULED,
        student_id=test_student.id,
    ).create()
    ...
    response = client.get(
        f"/appointments/{open_appointment.id}", headers=student_auth_headers
    )
    assert response.status_code == 200
    ...
    assert (
        response.json["appointment"]["appointment_time"]
        == open_appointment.appointment_time.isoformat()
    )
    ...
    assert (
        response.json["appointment"]["student_satisfaction"]
        == open_appointment.student_satisfaction
    )
    ...
    response = client.get(
        f"/appointments/{scheduled_appointment.id}", headers=student_auth_headers
    )
    assert response.status_code == 200
    assert response.json["appointment"]["id"] == str(scheduled_appointment.id)
    assert response.json["appointment"]["coach_id"] == str(
        scheduled_appointment.coach_id
    )
    assert (
        response.json["appointment"]["appointment_time"]
        == scheduled_appointment.appointment_time.isoformat()
    )
    assert response.json["appointment"]["status"] == scheduled_appointment.status.value
    assert response.json["appointment"]["student_id"] == str(
        scheduled_appointment.student_id
    )
    assert response.json["appointment"]["notes"] == scheduled_appointment.notes
    assert (
        response.json["appointment"]["student_satisfaction"]
        == scheduled_appointment.student_satisfaction
    )
    assert "coach_username" in response.json["appointment"]
...
def test_update_appointment(
    ...
    assert response.status_code == 400
    assert response.json == {
        "error": "Appointment cannot be open with student satisfaction"
    }
    ...
    assert response.status_code == 400
    assert response.json == {
        "error": "Appointment must be completed or scheduled with a student"
    }
    ...
    assert response.status_code == 400
    assert response.json == {
        "error": "Appointment cannot be scheduled with student satisfaction"
    }
    ...
    assert response.status_code == 400
    assert response.json == {
        "error": "Appointment must be completed with student satisfaction"
    }
...
def test_get_available_slots(
    ...
    assert all(
        [
            f"{slot.hour}:{slot.minute:02d}" in response.json["slots"]
    ...
    appointment_2 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 1, 11, 15, 0)
    ).create()
    # Next day
    appointment_4 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 2, 13, 15, 0)
    ).create()
    # Different coach
    appointment_5 = AppointmentFactory(
        coach_id=test_coach_2.id, appointment_time=datetime(2024, 1, 1, 13, 15, 0)
    ).create()
    ...
    appointment_6 = AppointmentFactory(
        coach_id=test_coach.id, appointment_time=datetime(2024, 1, 3, 11, 15, 0)
    ).create()
